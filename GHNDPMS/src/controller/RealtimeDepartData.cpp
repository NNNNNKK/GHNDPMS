...
{
	QJsonObject rtObj = DataStore::getRealTimeDataCache();//打开json文档
	QJsonObject orgObj = DataStore::getOrgnizeDataCache();
...
void RealtimeDepartData::service(HttpRequest &request, HttpResponse &response)
{
	...
	else
	{
		...
		QJsonObject robj = DataStore::getRealTimeDataCache();//获取缓存的实时数据
		QJsonObject org = DataStore::getOrgnizeDataCache(); //获取已有部门信息
		...
		page = page <= 1 ? 1 : page; //fix page range lower than 1 error.
