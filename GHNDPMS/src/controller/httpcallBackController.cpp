...
{
	QJsonObject backData = byteArray2json(request.getBody());//�ѷ��ص����ݽ�����json����
	...
	// qDebug() << "---------------------------------"<<backData;
	 ...
	 QFuture<void> f = QtConcurrent::run(dealBackdata, backData);//��ʱ�߳�
}
...
void dealBackdata(QJsonObject & ob)
{
	...
	QJsonObject event = ob["params"].toObject()["events"].toArray().first().toObject();//�¼�
	...
	std::string api = "/artemis/api/acs/v1/door/events";
	bool flag;
	for (int i = 0; i < 10; i++)
	{
           QThread::msleep(1000);
...
{
	QJsonObject  personMsg = HikdataRequest::httpRequest(HikdataRequest::gethikQuery(), api, hikrequest);//��ѯ�Ž�����¼�
	//Test
	//personMsg["code"] = 0;
	//
		...
		for (auto var : eventlist)
		{
			...
			if (subObj["cardNo"].toString() == virtualCard)
			{
				...
				if (ret) {
					...
					QJsonObject openJson = DataStore::getOrgnizeDataCache()[orgIndex].toObject();
					bool islocal=false;
					int contractor=50;
					if (!openJson.isEmpty()) {
                         islocal = openJson["islocal"].toBool();
					     contractor = openJson["orgEmployeesNumber"].toInt();
					}
					QJsonObject ps;
					...
					writeJson(orgIndex, orgName, pId, enterType, ps, islocal, contractor);//дjson
					//writePersonRealtimemsg(, orgIndex, QJsonObject &personMsg, DoorDirection status, QString orgName, int contractor, bool islocal, QString pId)
...
void writeJson(const QString & orgIndex, const QString & orgName, const QString &pId, const DoorDirection type, const QJsonObject & msg, bool islocal, int contractor)
{
	if (type == DoorDirection::Unused)
		return;
	QJsonObject priObj = DataStore::getRealTimeDataCache();
	QJsonObject updateObj = priObj[orgIndex].toObject();
	if (updateObj.isEmpty()) {
		if (type == DoorDirection::Import)
			updateObj["realnumber"] = int(1);
		else  if (type == DoorDirection::Export)
			updateObj["realnumber"] = int(0);
		//priObj[orgIndex] = psdetail;
	} else {
		if (type == DoorDirection::Import)
			updateObj["realnumber"]= updateObj["realnumber"].toInt()+1;
		else  if (type == DoorDirection::Export)
			updateObj["realnumber"] = updateObj["realnumber"].toInt() - 1;
		auto updatePersonObj = updateObj[pId].toObject();
	}
   updateObj["islocal"] = islocal;
   updateObj["contractor"] = contractor;
   updateObj["orgIndexCode"] = orgIndex;
   updateObj["orgName"] = orgName;
   updateObj[pId] = msg;
   priObj[orgIndex] = updateObj;
	//auto ite= priObj.find(orgIndex);
	//if (ite == priObj.end())
	//{
	//	QJsonObject psdetail;
	//	psdetail[pId] = msg;//��Ա���¼�¼
	//	psdetail["orgIndexCode"] = orgIndex;//��֯Ψһ��ʶ��
	//	psdetail["orgName"] = orgName;//��֯����
	//	if (type == DoorDirection::Import)
 ...
 //          (*subite)=msg;
	...
	//	(*ite) = ob;
	//}
	DataStore::setRealTimeDataCache(priObj);
}
