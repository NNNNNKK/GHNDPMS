...
{
	QJsonObject  && ob = DataStore::getRealTimeDataCache();//打开原来数据
	...
	DataStore::setRealTimeDataCache(ob);
}
...
void HikdataRequest::historyDataDeal(QString startDate, QString orgCode)
{
	...
	QJsonObject orgObject = DataStore::getOrgnizeDataCache();//读取部门信息
...
void HikdataRequest::realtimeDataDeal()
{
	...
	OrgDoc = DataStore::getOrgnizeDataCache();//获取部门
